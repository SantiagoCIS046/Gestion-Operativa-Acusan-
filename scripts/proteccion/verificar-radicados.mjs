// ============================================================================
// VERIFICADOR DE INTEGRIDAD — MÓDULO RADICADOS (CONGELADO)
// ============================================================================
// El módulo de Radicados (frontend + backend) se declaró TERMINADO y estable.
// Este script compara el contenido actual de los archivos protegidos contra
// el registro de hashes `radicados.lock.json` (la "foto" congelada).
// Diseño replicado del congelamiento de Permisos (revisión adversarial
// 2026-09-01), aplicado a Radicados el 2026-09-03.
//
// Uso:
//   node scripts/proteccion/verificar-radicados.mjs            → verificar disco (exit 1 si derivó)
//   node scripts/proteccion/verificar-radicados.mjs --avisar   → solo avisa, nunca falla
//   node scripts/proteccion/verificar-radicados.mjs --staged   → verificar el INDEX de git
...
// hooks la fijan explícitamente (RADICADOS_RAIZ) porque ejecutan una copia del
// verificador extraída de HEAD, que vive dentro de .git/.
const RAIZ = process.env.RADICADOS_RAIZ
  ? process.env.RADICADOS_RAIZ.replace(/\\/g, '/').replace(/\/$/, '') + '/'
  : fileURLToPath(new URL('../../', import.meta.url))
const LOCK_REL = 'scripts/proteccion/radicados.lock.json'
...
const DIRS_PROTEGIDOS = [
  'acusan/frontend/src/modules/radicados',
  'acusan/backend/src/modules/radicados',
]
...
const fallarCerrado = (mensaje) => {
  console.error(`${ROJO}✖ PROTECCIÓN RADICADOS: ${mensaje}${RESET}`)
  console.error(`  La verificación se aborta (fail-closed): no se emite un "íntegro" sin haber comparado.`)
  console.error(`  Referencia de restauración: git checkout radicados-estable-v1 -- ${DIRS_PROTEGIDOS.join(' ')}`)
  process.exit(1)
}
...
function lockDesdeHEAD() {
  ...
  if (r.status !== 0) {
    fallarCerrado(`no se pudo leer el lock confirmado en HEAD (¿historial anterior al congelamiento?). ` +
      `Restaure con: git checkout radicados-estable-v1 -- ${LOCK_REL}`)
  }
...
function parsearLock(texto, origen) {
  try {
    ...
    fallarCerrado(`el lock (${origen}) está ausente, corrupto o con BOM. ` +
      ...
      `(RADICADOS_DESBLOQUEAR=1 ... --generar); si no, restaure desde git.`)
  }
}
...
if (MODO === '--generar') {
  if (process.env.RADICADOS_DESBLOQUEAR !== '1') {
    console.error(`${ROJO}✖ Para regenerar el lock de Radicados use RADICADOS_DESBLOQUEAR=1 (desbloqueo intencional).${RESET}`)
    console.error(`  Si no sabe por qué está esto: alguien modificó el módulo congelado. Revise con:`)
    console.error(`  git diff radicados-estable-v1 -- ${DIRS_PROTEGIDOS.join(' ')}`)
    process.exit(1)
  }
  const archivos = {}
  ...
  const lock = {
    version: 1,
    descripcion: 'Módulo Radicados congelado — estado estable verificado',
    algoritmo: 'sha256',
  ...
  console.log(`${VERDE}✔ Lock de Radicados regenerado: ${Object.keys(archivos).length} archivos registrados.${RESET}`)
  console.log(`  Recuerde commitar el cambio y el lock JUNTOS y con RADICADOS_DESBLOQUEAR=1 activo.`)
  process.exit(0)
}
...
if (problemas.length === 0) {
  if (MODO !== '--avisar') {
    console.log(`${VERDE}✔ Módulo Radicados íntegro (${totalRegistrados} archivos coinciden con el lock${MODO === '--staged' ? ' de HEAD' : ''}).${RESET}`)
  }
  process.exit(0)
}
...
const encabezado =
  `${ROJO}╔══════════════════════════════════════════════════════════════════╗${RESET}\n` +
  `${ROJO}║  ⚠  MÓDULO RADICADOS MODIFICADO — ESTÁ CONGELADO (YA ESTABLE) ⚠  ║${RESET}\n` +
  `${ROJO}╚══════════════════════════════════════════════════════════════════╝${RESET}\n` +
  ...
  `    git checkout radicados-estable-v1 -- ${DIRS_PROTEGIDOS.join(' ')}\n\n` +
  `  ${CYAN}Si el cambio es INTENCIONAL y aprobado:${RESET}\n` +
  `    1. RADICADOS_DESBLOQUEAR=1 node scripts/proteccion/verificar-radicados.mjs --generar\n` +
  `    2. Commitar juntos el cambio y el lock, con RADICADOS_DESBLOQUEAR=1 aún activo\n\n` +
  `  Documentación completa: PROTECCION-RADICADOS.md (raíz del repo)\n`
